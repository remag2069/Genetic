...
def undo(event):
    ...
    for j in range(i,len(pos_x)-1):
        canvas.create_polygon(pos_x[j],pos_y[j],pos_x[j+1],pos_y[j+1],fill=bg_color[bg_code])
    for j in range(i,len(pos_x)-1):
        pos_x.pop()
        pos_y.pop()
...
def left(event):
    ...
    pos_x.append(pos_x1)
    pos_y.append(pos_y1)
    canvas.create_polygon(pos_x1,pos_y1,pos_x2,pos_y2, fill=color[color_code])
    pos_x1 = pos_x2
    pos_y1 = pos_y2
